...
type MigrateColorDestination struct {
	...
	OlseraSecretKey   	string     	`gorm:"type:varchar(100);" json:"olsera_secret_key"`
	OlseraAccessToken   *string     	`gorm:"type:text" json:"-"`
	OlseraRefreshToken  *string     	`gorm:"type:text" json:"-"`
	OlseraTokenExpiresAt  *time.Time     	`json:"-"`
...
func (d *MigrateColorDestination) IsTokenExpired() bool {
	if d.OlseraTokenExpiresAt == nil {
		return true	
	}

	return time.Now().After(*d.OlseraTokenExpiresAt)
}
